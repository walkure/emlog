...
DKMS packaging is defined in `dkms.conf` (`make dkms_install` / `make dkms_remove`) for auto-rebuilding the module across kernel upgrades.
...
- `emlog_init`/`emlog_remove` register a char device region + `class`/`device` so `/dev/emlog` auto-appears under devtmpfs/udev with a minimal default buffer
...
Single-file FUSE 2.x filesystem. Maintains its own in-memory table of open logical files (`g_files`) and file handles (`g_fh`), each logical file backed by a real emlog character device node created on demand in a hidden directory (`g_dev_dir`, default `/dev/.emlog_fuse_devs`) via `mknod` (`ensure_device`), using the major number auto-discovered from `/proc/devices` (`get_emlog_major`) and a per-mount buffer size (`-o buffer_size=N` KB). FUSE ops (`emfuse_*`) translate filesystem calls into read/write/open/close against that backing device; `emfuse_release` removes the backing device once all handles to a logical file are closed. `check_capability`/`preflight` gate the `CAP_MKNOD`/`CAP_CHOWN` requirements for non-root operation (compiled out via `-DNO_LIBCAP` when libcap isn't available).
